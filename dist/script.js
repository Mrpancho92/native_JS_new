...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock == '' || materialBlock.value == '') {
      resultBlock.textContent = "Please, choose size and material picture";
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector, destroy = false) {
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        // if (e.target.className === 'fixed-gift wow pulse infinite') {
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  //         card.classList.add('col-sm-3', 'col-sm-offset-0', 'ol-xs-10', 'col-xs-offset-1');
  //     });
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('assets/db2.json').then(res => createCards(res.styles)).catch(error => console.log(error));
    this.remove();
  });
  function createCards(response) {
    response.forEach(({
      ...
      card.classList.add('animated', 'fadeInUp', 'col-sm-3', 'col-sm-offset-0', 'ol-xs-10', 'col-xs-offset-1');
      card.innerHTML = `
